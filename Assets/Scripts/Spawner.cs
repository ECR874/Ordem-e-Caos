...
public class Spawner : MonoBehaviour
{
    ...
    {
        Enemy.OnEnemyReachedEnd += EnemyGoToEnd;
    }
    ...
    void Update()
    {
        ...
        else
        {
            ...
            if (_spawnTime <= 0 && _spawnCounter < CurrentWave.numberOfEnemies)
            {
                _spawnTime = CurrentWave.spawnInterval;
                SpawnEnemy();
                _spawnCounter++;
            }
            else if (_spawnCounter >= CurrentWave.numberOfEnemies && _enemiesRemoved >= CurrentWave.numberOfEnemies)
            {
                _betweenWaves = true;
            }
        }
    ...
    private void EnemyGoToEnd(EnemyData data)
    {
        _enemiesRemoved++;
    }
}
